...
def train(rank, args):
    ...
    if resume_flag:
        ...
        for l in model_list:
            idx_rank.append(
                int(l.split('/')[-1].split('_')[-2][5:]) * 21000 + int(l.split('/')[-1].split('_')[-1].split('.')[0]))
    ...
    data_loader_val = DataLoader(my_dataset_val, batch_size=batch_size, num_workers=8, prefetch_factor=4,
                                 drop_last=True)
        ...
        for i, data in enumerate(data_loader):
            ...
            if skip_step:
                ...
                if args.gpus == 1 or (args.gpus > 1 and dist.get_rank() == 0):
                    print('rank: ', rank, 'time:', time.time() - s2, 's', ' epoch: ', epoch, i, 'loss:',
                          final_loss.item())
                if (i + 1) % 400 == 0:
                ...
                if i % 4000 == 0:
                    with torch.no_grad():
                        ...
                        for j, dataval in enumerate(data_loader_val):
                            prompt = data['caption']
                            model_kwargs = dict(texts=prompt)
                            t = torch.randint(0, DIFFUSION_CONFIGS[base_name]['timesteps'], size=(batch_size,),
                                              device=device)
                            x_start = data['pts'].cuda()
                            loss = diffusion.training_losses(model, x_start, t, model_kwargs=model_kwargs)
                            final_loss = torch.mean(loss['loss'])
...
if __name__ == '__main__':
    ...
    model_group.add_argument('--pts_pt_path', type=str, default='./Cap3D_pcs_pt',
                             help='the directory to the .pt file which store point clouds')
